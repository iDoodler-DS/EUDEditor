...
Module BGMPlayerModule
    Public Function GetBGMPlyereps() As String
        ...
        str.AppendLine("const musicLastTime = PVariable();")
        str.AppendLine("const musicFrame = EUDArray(8);")
        str.AppendLine("const musicnum = EUDArray(8);")
        str.AppendLine("const musicflag = EUDArray(8);")
        str.AppendLine("const musicisplay = EUDArray(8);")
        ...
        str.AppendLine("")
        str.AppendLine("            } else {")
        str.AppendLine("                // when the sound finished playing")
        str.AppendLine("                if (musicflag[i] == 1) {")
        str.AppendLine("                    // when repeat is enabled")
        ...
        str.AppendLine()
        str.AppendLine("function parsePlayer(tplayer: TrgPlayer) {")
        ...
        str.AppendLine("}")
        str.AppendLine()
        str.AppendLine("function Play(tplayer: TrgPlayer, BGMNum, flag) {")
        str.AppendLine("    const p = parsePlayer(tplayer);")
        str.AppendLine("    if (flag == 0 && musicflag[p] == 0) {")
        str.AppendLine("        if (musicnum[p] != BGMNum) {")
        ...
        str.AppendLine("}")
        str.AppendLine()
        str.AppendLine("function Stop(tplayer: TrgPlayer) {")
        str.AppendLine("    const p = parsePlayer(tplayer);")
        str.AppendLine("    musicisplay[p] = 0;")
        str.AppendLine("}")
        str.AppendLine()
        str.AppendLine("function Resume(tplayer: TrgPlayer) {")
        str.AppendLine("    const p = parsePlayer(tplayer);")
        str.AppendLine("    musicisplay[p] = 1;")
        str.AppendLine("}")
        str.AppendLine()
        str.AppendLine("function CurrentPlayMusic(tplayer: TrgPlayer) {")
        str.AppendLine("    const p = parsePlayer(tplayer);")
        str.AppendLine("    return musicnum[p];")
        str.AppendLine("}")
        str.AppendLine()
        str.AppendLine("function isplaying(tplayer: TrgPlayer) {")
        str.AppendLine("    const p = parsePlayer(tplayer);")
