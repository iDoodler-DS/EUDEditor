Public Class GRPFormUseGRP
 ...
 Handles ListBox1.DrawItem
    ...
    Private Sub ListDraw()
        ...
        For i = 0 To CODE(listNum).Count - 1
            ...
            If temp <> "None" Then
                ...
                Dim imagefile As Integer
                Select Case listnumber
                    Case 0
                        Dim unitgrp As Integer = DatEditDATA(DTYPE.units).ReadValue("Graphics", i)
                        Dim sprite As Integer = DatEditDATA(DTYPE.flingy).ReadValue("Sprite", unitgrp)
                        imagefile = DatEditDATA(DTYPE.sprites).ReadValue("Image File", sprite)

                    Case 2
                        Dim sprite As Integer = DatEditDATA(DTYPE.flingy).ReadValue("Sprite", i)
                        imagefile = DatEditDATA(DTYPE.sprites).ReadValue("Image File", sprite)

                    Case 3
                        imagefile = DatEditDATA(DTYPE.sprites).ReadValue("Image File", i)
                ...
                If GRPEditorUsingDATA(imagefile) = "" Then
                    ' GRPFormUseGRP(0)
    ...
    Private Sub ListBox1_SelectedIndexChanged(sender As Object, e As EventArgs) Handles ListBox1.SelectedIndexChanged
        If ListBox1.Items.Count <> 0 Then
            ...
            Select Case listnumber
                ...
                Case 4
                    returnvalue = fristValue
            End Select

            drawUnitGRP()
            Text = "Selectimage   FrameCount : " & grpcount & "    Current : " & currentcount
            If grpcount < currentcount Then

                If fristwarring = False Then
                    MsgBox(Lan.GetText("GRPFormUseGRP", "ErrorMsg"), MsgBoxStyle.Critical, ProgramSet.AlterFormMessage)
                    fristwarring = True
                End If
    ...
    Private Sub drawUnitGRP()
        ...
        ICONGRP.LoadGRP(mpq.ReaddatFile("unit\" & CODE(DTYPE.grpfile)(DatEditDATA(DTYPE.images).ReadValue("GRP File", imagevalue)).Replace("<0>", ""))) 'unit\protoss\dragoo
        currentcount = ICONGRP.framecount
